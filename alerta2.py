...
# Se não mudaste a pass, o user/pass pode não ser necessário se a segurança estiver off no dev
ES_USER = "elastic" 
ES_PASS = "DkHgTURj1qghpxqNC3Vi" 

EMAIL_ORIGEM = "sofia.c.fernandes03@gmail.com"
EMAIL_PASSWORD = "pvqn ljgr fftr mpmg" 
EMAIL_DESTINO = "josedanielsantos@gmail.com"

LIMITE_TEMPERATURA = 25.0
INTERVALO_MINUTOS = 5  
INDEX_NAME = "logstash-*" # <--- IMPORTANTE: Deve corresponder ao padrão do teu Logstash

def verificar_temperatura():
    print("--- A verificar temperaturas... ---")
    
    # 1. Liga ao elasticsearch
    try:
    ...
    # 2. Define o intervalo de tempo (Agora menos 5 minutos)
    # Nota: O Elastic costuma usar UTC.
    ...
    # 3. A Pergunta ao Elastic (Query CORRIGIDA)
    query = {
        "bool": {
            "must": [
                # A Procura deve ser feita no campo 'temperature' (ou 'sensor.temperature')
                {"range": {"temperature": {"gt": LIMITE_TEMPERATURA}}}, 
                # O Logstash normalmente grava o tempo em '@timestamp'
    ...
    try:
        ...
        # Em algumas versões do elastic, value é um dict, noutras é int.
        ...
        if hits > 0:
            ...
            # Tenta ir buscar o valor. Se falhar, mete 'Desconhecido'
            valor_lido = dado.get('temperature', 'N/A')
            sensor = dado.get('sensor', 'Sensor Desconhecido') # O teu script gerador usa "sensor" ou não envia ID?
